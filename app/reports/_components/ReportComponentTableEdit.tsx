...
const PILL_BASE_COLOR_OPTIONS = BASE_APP_COLOR_KEYS;
const PILL_SEMANTIC_COLOR_SWATCH: Record<(typeof PILL_SEMANTIC_COLOR_OPTIONS)[number], string> = {
...
function getPillOptionColorHex(colorName: string): string {
  if (colorName in PILL_SEMANTIC_COLOR_SWATCH) {
    return PILL_SEMANTIC_COLOR_SWATCH[colorName as keyof typeof PILL_SEMANTIC_COLOR_SWATCH];
  }
  if (colorName in APP_COLORS) {
    return APP_COLORS[colorName as keyof typeof APP_COLORS];
  }
...
export default function ReportComponentTableEdit(args: { reportId: string; reportComponentId: string }) {
  ...
  const lastSavedSelectedSignatureRef = useRef<string>("");
  ...
  const loadConfig = useCallback(async () => {
    ...
    try {
      ...
      lastSavedSelectedSignatureRef.current = JSON.stringify(nextSelected);
  ...
  }, [availableColumns, reportComponentId, reportId, selectedColumns]);
  ...
  }, [accessorBySelectedKey, availableByKey, columnTypeDrafts, selectedColumns, totalsBySelectedKey]);
  ...
  const selectedSignature = useMemo(() => JSON.stringify(selectedColumns), [selectedColumns]);

  useEffect(() => {
    if (loading || saving || !reportRoute) {
      return;
    }
    if (selectedSignature === lastSavedSelectedSignatureRef.current) {
      return;
    }
    ...
    autoSaveTimerRef.current = setTimeout(() => {
      void (async () => {
        try {
          ...
          const json = (await res.json()) as TableConfigResponse;
          if (!res.ok || !json.ok) {
            throw new Error(json.error || "Failed to sync selected columns");
          }

          lastSavedSelectedSignatureRef.current = JSON.stringify(filteredSelected);
          await loadPreview(reportRoute);
        } catch (e: unknown) {
          setError(String(e));
        }
      })();
    }, 250);
  }, [availableColumns, loading, loadPreview, reportComponentId, reportId, reportRoute, saving, selectedColumns, selectedSignature]);
                        ...
                        {activePillValues.map((value) => (
                          ...
                          >
                            ...
                            <select
                              ...
                              onChange={(event) =>
                                updatePillValueColor(activeColumn.key, value.key, event.target.value)
                              }
                              className="rounded border px-2 py-1 text-sm"
                              style={{
                                borderColor: "var(--app-border)",
                            ...
                            >
                              ...
                              <optgroup label="Semantic">
                                {PILL_SEMANTIC_COLOR_OPTIONS.map((colorName) => (
                                  <option
                                    key={`semantic-${colorName}`}
                                    value={colorName}
                                    style={{ color: getPillOptionColorHex(colorName) }}
                                  >
                                    {`● ${colorName}`}
                                  </option>
                                ))}
                              ...
                              <optgroup label="Base Colors">
                                {PILL_BASE_COLOR_OPTIONS.map((colorName) => (
                                  <option
                                    key={`base-${colorName}`}
                                    value={colorName}
                                    style={{ color: getPillOptionColorHex(colorName) }}
